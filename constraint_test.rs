...
impl Expression {
    ...
    fn evaluate(&self, context: &dyn EvaluationContext) -> Result<f64> {
        ...
        match self.expr.as_str() {
            ...
            "x" => context.get_variable("x").map_err(|_| Error::ExpressionError("Variable x not found".to_string())),
            "y" => context.get_variable("y").map_err(|_| Error::ExpressionError("Variable y not found".to_string())),
            
            // Operations
            "x - y" => {
                let x = context.get_variable("x").map_err(|_| Error::ExpressionError("Variable x not found".to_string()))?;
                let y = context.get_variable("y").map_err(|_| Error::ExpressionError("Variable y not found".to_string()))?;
                Ok(x - y)
            },
            "y - x" => {
                let x = context.get_variable("x").map_err(|_| Error::ExpressionError("Variable x not found".to_string()))?;
            ...
            // Anything else
            _ => Err(Error::ExpressionError(format!("Unsupported expression: {}", self.expr))),
        }
    }
...
impl Constraint {
    fn new(lhs: &str, constraint_type: ConstraintType, rhs: &str) -> Result<Self> {
        ...
        let combined_expr = match constraint_type {
            ConstraintType::LessThan | ConstraintType::LessThanOrEqual => format!("{} - {}", lhs, rhs),
            ConstraintType::GreaterThan | ConstraintType::GreaterThanOrEqual => format!("{} - {}", rhs, lhs),
            _ => format!("{} - {}", lhs, rhs),
        };
    ...
    fn is_satisfied(&self, context: &dyn EvaluationContext) -> Result<bool> {
        if let Some(expr) = &self.expr {
            ...
            Err(Error::ConstraintError("Expression not initialized".to_string()))
        }
    }
...
impl Constraints {
    ...
    fn add_constraint(&mut self, lhs: &str, constraint_type: ConstraintType, rhs: &str) -> Result<()> {
...
impl Parameters {
    ...
    fn add_param(&mut self, name: &str, value: f64) -> Result<()> {
        ...
        self.deps.entry(name.to_string()).or_insert_with(HashSet::new);
        Ok(())
    }
    ...
    fn update_expressions(&mut self) -> Result<()> {
        ...
        let expr_params: Vec<_> = self.params.keys()
