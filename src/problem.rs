...
pub mod lm_compat {
    ...
    use levenberg_marquardt::{LeastSquaresFunc, LeastSquaresProblem};
        ...
        >,
    {
        fn eval(&self, params: &Array1<f64>) -> Result<Array1<f64>> {
            ...
            let nalgebra_residuals = self.problem.residuals(&nalgebra_params).map_err(|e| {
                LmOptError::FunctionEvaluation(format!("Error in LM problem residuals: {:?}", e))
            })?;
        ...
        fn jacobian(&self, params: &Array1<f64>) -> Result<Array2<f64>> {
            ...
            let nalgebra_jacobian = self.problem.jacobian(&nalgebra_params).map_err(|e| {
                LmOptError::FunctionEvaluation(format!("Error in LM problem jacobian: {:?}", e))
            })?;
...
pub mod lm_adapter {
    ...
    impl<P: Problem> LeastSquaresProblem<f64> for LmAdapter<P> {
        ...
        fn residuals(
            &self,
            params: &Self::Params,
        ) -> Result<Self::Residuals, levenberg_marquardt::Error> {
        ...
        fn jacobian(
            &self,
            params: &Self::Params,
        ) -> Result<Self::Jacobian, levenberg_marquardt::Error> {
            // Convert nalgebra params to ndarray
            let ndarray_params = nalgebra_to_ndarray(params)
                .map_err(|e| levenberg_marquardt::Error::InvalidInputs(format!("{}", e)))?;

            // Call our problem's jacobian method
            let ndarray_jacobian = self
                .problem
                .jacobian(&ndarray_params)
                .map_err(|e| levenberg_marquardt::Error::InvalidInputs(format!("{}", e)))?;
