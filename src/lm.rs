...
pub mod utils {
    ...
    pub fn check_convergence(
        ...
        let param_change_vec: Vec<f64> = new_params
            .iter()
            .zip(params.iter())
            .map(|(a, b)| (a - b).abs())
            .collect();
        let param_change = param_change_vec
            .iter()
            .fold(0. / 0., |a, &b| f64::max(a, b));
        if param_change < config.xtol {
            return Some(format!(
                "Parameter convergence: |dx|/|x| = {:.2e} < {:.2e}",
                param_change, config.xtol
            ));
        }
    ...
    pub fn update_lambda(lambda: f64, success: bool, config: &LmConfig) -> f64 {
        if success {
