...
use crate::parameters::expression::{Expression, ExpressionError, EvaluationContext};
use crate::parameters::constraints::{Constraint, ConstraintType, Constraints, ConstraintError};
use std::ops::Sub;
...
impl Parameters {
    ...
    pub fn update_from_internal(&mut self, values: &[f64]) -> Result<(), ParameterError> {
        let varying: Vec<_> = self.params.iter()
            .filter(|(_, p)| p.vary())
        ...
        // First, update all varying parameters
        ...
        if !self.constraints.is_empty() {
            ...
            if !satisfied {
                ...
                let violated = self.violated_constraints()?;
                let violations = violated.iter()
                    .map(|c| c.to_string())
                    .collect::<Vec<_>>()
                    .join(", ");
    ...
    fn update_expressions(&mut self) -> Result<(), ParameterError> {
        // Get a list of parameters with expressions
        let params_with_expr: Vec<_> = self.params.keys()
        ...
        let sorted_expr_params: Vec<_> = sorted_params.into_iter()
    ...
    fn topological_sort(&self) -> Result<Vec<String>, ParameterError> {
        ...
        for name in self.params.keys() {
            if !visited.contains(name) {
                visit(name, &self.deps, &mut visited, &mut temp_visited, &mut result)?;
            }
        }
    ...
    fn check_parameter_accessible(&self, param_name: &str, expr_param_name: &str) -> Result<bool, ParameterError> {
    ...
    pub fn get_dependencies(&self, name: &str) -> Result<Vec<String>, ParameterError> {
        let param = self.params.get(name).ok_or_else(|| ParameterError::ParameterNotFound {
            name: name.to_string(),
        })?;
    ...
    /// ```
    pub fn add_constraint(&mut self, lhs: &str, constraint_type: ConstraintType, rhs: &str) -> Result<(), ParameterError> {
...
mod tests {
    ...
    fn test_add_parameter() {
        ...
        params.add_param_with_bounds("sigma", 2.0, 0.1, 10.0).unwrap();
    ...
    fn test_reset_parameters() {
        ...
        params.get_mut("amplitude").unwrap().set_value(15.0).unwrap();
    ...
    fn test_expression_evaluation() {
        ...
        assert_eq!(params.eval_expression("x^2 + 2*y + z/2").unwrap(), 10.0);
    ...
    fn test_dependency_tracking() {
        ...
        params.add_param_with_expr("complex", 14.0, "sum + product").unwrap();
    ...
    fn test_update_expressions() {
        ...
        params.add_param_with_expr("complex", 0.0, "sum + product").unwrap();
    ...
    fn test_update_from_internal_with_expressions() {
        ...
        // Get internal values for varying parameters
        let internal = params.varying_internal_values().unwrap();
        assert_eq!(internal.len(), 2);
        
        // Update with new internal values
        let new_values = vec![4.0, 5.0];
        params.update_from_internal(&new_values).unwrap();
        
        // Check that base parameter values were updated
        assert_eq!(params.get("x").unwrap().value(), 4.0);
        assert_eq!(params.get("y").unwrap().value(), 5.0);
        
        // Check that expression parameter was also updated
        assert_eq!(params.get("sum").unwrap().value(), 9.0);
    }
    ...
    fn test_constraints_basic() {
        ...
        // Add constraints
        params.add_constraint("x", ConstraintType::LessThan, "y").unwrap();
        params.add_constraint("x", ConstraintType::GreaterThan, "0").unwrap();
    ...
    fn test_constraint_with_expressions() {
        ...
        params.add_constraint("x + y", ConstraintType::LessThan, "z").unwrap();
    ...
    fn test_constraint_with_update_from_internal() {
        ...
        // Verify we have the correct parameters before updating
        params.varying_internal_values().unwrap();
        
        // Update with values that satisfy the constraint
        let new_values = vec![1.0, 5.0];
        assert!(params.update_from_internal(&new_values).is_ok());
        
        // Update with values that violate the constraint
        let new_values = vec![10.0, 5.0];
        assert!(params.update_from_internal(&new_values).is_err());
    }
    ...
    fn test_constraints_with_parameter_expressions() {
        ...
        // Add a constraint
        params.add_constraint("sum", ConstraintType::LessThan, "10").unwrap();
