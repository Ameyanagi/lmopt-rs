#[cfg(test)]
mod tests {
    use crate::parameters::{Parameter, Parameters, Bounds, BoundsTransform};
    ...
    fn test_bounds_transform() {
        ...
        for &internal in &internal_values {
            ...
            if bounds.is_within_bounds(external) {
                ...
                assert!((internal - internal_round_trip).abs() < test_tolerance,
                        "Failed for internal value: {}, round-trip diff: {}",
                        internal, (internal - internal_round_trip).abs());
            }
        }
    }
}