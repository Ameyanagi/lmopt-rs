...
use ndarray::{Array1, Array2}; // Removed LmOptError since it's not used
...
///
/// * `LmOptError::ConversionError` if the conversion fails
pub fn nalgebra_vec_to_faer<T: NumericScalar + nalgebra::Scalar>(
    vec: &DVector<T>,
) -> Result<Col<T>> {
    // Get dimension
    let n = vec.nrows();
...
///
/// * `LmOptError::ConversionError` if the conversion fails
pub fn faer_vec_to_nalgebra<T: NumericScalar + nalgebra::Scalar>(
    col: &Col<T>,
) -> Result<DVector<T>> {
    // Get dimension
    let n = col.nrows();
