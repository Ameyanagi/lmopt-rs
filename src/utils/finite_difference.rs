...
pub fn jacobian(
    ...
    if residuals.len() != n_residuals {
        return Err(LmOptError::DimensionMismatch(format!(
            "Expected {} residuals, got {}", n_residuals, residuals.len()
        )));
    }
...
mod tests {
    ...
    fn test_gradient() {
        ...
        // Analytical gradient: [2*x + y, 4*y + x] = [5, 14]
        let grad = gradient(test_function, &params, None).unwrap();
        
        assert_eq!(grad.len(), 2);
        assert_relative_eq!(grad[0], 5.0, epsilon = 1e-5);
        assert_relative_eq!(grad[1], 14.0, epsilon = 1e-5);
    }
    ...
    fn test_hessian() {
        ...
        let hess = hessian(test_function, &params, None).unwrap();
        
        assert_eq!(hess.shape(), &[2, 2]);
        assert_relative_eq!(hess[[0, 0]], 2.0, epsilon = 1e-5);
        assert_relative_eq!(hess[[0, 1]], 1.0, epsilon = 1e-5);
        assert_relative_eq!(hess[[1, 0]], 1.0, epsilon = 1e-5);
        assert_relative_eq!(hess[[1, 1]], 4.0, epsilon = 1e-5);
    }
