...
impl Problem for ArrheniusModel {
    fn eval(&self, params: &Array1<f64>) -> Result<Array1<f64>, LmOptError> {
        ...
        let predicted = self
            .temperature
            .mapv(|temp| Self::arrhenius(temp, ln_a, ea));
        Ok(&predicted - &self.rate_constant)
    }
...
fn test_arrhenius_model() {
    ...
    let rate_constants =
        temperatures.mapv(|temp| ArrheniusModel::arrhenius(temp, true_ln_a, true_ea));
    ...
    if let Ok(solution) = result {
        ...
        assert!(
            (optimized_params[0] - true_ln_a).abs() < ln_a_tolerance,
            "ln(A) doesn't match: got {}, expected {}",
            optimized_params[0],
            true_ln_a
        );

        assert!(
            (optimized_params[1] - true_ea).abs() < ea_tolerance,
            "Ea doesn't match: got {}, expected {}",
            optimized_params[1],
            true_ea
        );

        println!(
            "Arrhenius model test passed! Parameters: ln(A) = {}, Ea = {} J/mol",
...
impl Problem for PharmacokineticModel {
    fn eval(&self, params: &Array1<f64>) -> Result<Array1<f64>, LmOptError> {
        ...
        let predicted = self
            .time
            .mapv(|t| Self::two_compartment(t, a, alpha, b, beta));
        Ok(&predicted - &self.concentration)
    }
...
fn test_pharmacokinetic_model() {
    ...
    let concentrations = time_points
        .mapv(|t| PharmacokineticModel::two_compartment(t, true_a, true_alpha, true_b, true_beta));
    ...
    if let Ok(solution) = result {
        ...
        assert!(
            (optimized_params[0] - true_a).abs() / true_a < tolerance,
            "A doesn't match: got {}, expected {}",
            optimized_params[0],
            true_a
        );

        assert!(
            (optimized_params[1] - true_alpha).abs() / true_alpha < tolerance,
            "alpha doesn't match: got {}, expected {}",
            optimized_params[1],
            true_alpha
        );

        assert!(
            (optimized_params[2] - true_b).abs() / true_b < tolerance,
            "B doesn't match: got {}, expected {}",
            optimized_params[2],
            true_b
        );

        assert!(
            (optimized_params[3] - true_beta).abs() / true_beta < tolerance,
            "beta doesn't match: got {}, expected {}",
            optimized_params[3],
            true_beta
        );

        println!(
            "Pharmacokinetic model test passed! Parameters: A = {}, alpha = {}, B = {}, beta = {}",
