...
fn test_quadratic_fit() {
    ...
    assert_relative_eq!(result.params[0], 2.0, epsilon = 0.1);  // a ≈ 2
    assert_relative_eq!(result.params[1], -3.0, epsilon = 0.1); // b ≈ -3
    assert_relative_eq!(result.params[2], 1.0, epsilon = 0.1);  // c ≈ 1
    
    // Check that cost function is small
    assert!(result.cost < 0.5);  // Allow for noise in the data
}
...
fn test_multiple_starting_points() {
    ...
    for (i, initial_params) in starting_points.iter().enumerate() {
        ...
        assert_relative_eq!(result.params[0], 2.0, epsilon = 1e-4, 
                            max_relative = 1e-4, 
                            epsilon_message = &format!("Starting point {}: a parameter incorrect", i));
        assert_relative_eq!(result.params[1], 3.0, epsilon = 1e-4, 
                            max_relative = 1e-4,
                            epsilon_message = &format!("Starting point {}: b parameter incorrect", i));
    }
}
...
fn test_damping_parameter() {
    ...
    assert_relative_eq!(result_small.params[0], result_large.params[0], epsilon = 1e-4);
    assert_relative_eq!(result_small.params[1], result_large.params[1], epsilon = 1e-4);
}