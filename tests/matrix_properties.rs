...
use lmopt_rs::utils::matrix_convert::{
    faer_to_nalgebra, faer_to_ndarray, faer_vec_to_ndarray, nalgebra_to_faer, nalgebra_to_ndarray,
    nalgebra_vec_to_ndarray, ndarray_to_faer, ndarray_to_nalgebra, ndarray_vec_to_faer,
    ndarray_vec_to_nalgebra,
};
...
fn property_matrix_addition_commutative() {
    ...
    for _ in 0..10 {
        // Run multiple tests with different random matrices
        ...
        for i in 0..rows {
            for j in 0..cols {
                assert_relative_eq!(
                    a_plus_b_from_faer[[i, j]],
                    b_plus_a_from_faer[[i, j]],
                    epsilon = 1e-10
                );
            }
        }
...
fn property_matrix_addition_associative() {
    ...
    for _ in 0..10 {
        ...
        for i in 0..rows {
            for j in 0..cols {
                assert_relative_eq!(
                    ab_plus_c_from_faer[[i, j]],
                    a_plus_bc_from_faer[[i, j]],
                    epsilon = 1e-10
                );
            }
        }
...
fn property_ill_conditioned_matrix_stability() {
    ...
    match solve_result {
        Ok(x_nalgebra) => {
            ...
            println!(
                "Residual norm for ill-conditioned system: {}",
                residual_norm
            );
...
fn property_consistent_linear_system() {
    ...
    let svd = a_nalgebra.clone().svd(true, true);
    let x_nalgebra = svd
        .solve(&b_nalgebra, 1e-10)
        .expect("SVD should solve this consistent system");
    let x = nalgebra_vec_to_ndarray(&x_nalgebra).unwrap();
